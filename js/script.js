...
c('.modelsInfo--addButton').addEventListener('click', () => {
    ...
    // Lógca para adicionar o item ao carrinho
...
function updateCart(){
    ...
    if(cart.length > 0){
        // Adicionar a classe 'show para exibir a barra lateral no carrinho
        ...
        cart.map((itemCart, index)=>{
            // Encontrar o item correspondente nos dados nos modelos
            ...
            cartItem.querySelector('.cart--item--nome').innerHTML = `${modelsItem.name} - ${modelsItem.sizes[itemCart.size]}`;
            cartItem.querySelector('.cart--item--qt').innerHTML = itemCart.qt;

            // Adicionar eventos de clique para diminuir e aumntar a quantidade do item no carrinho
